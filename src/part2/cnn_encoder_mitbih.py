...
def load_train_test(dpath="./data/ptbdb/"):
    ...
    y_train = df_train.iloc[:, -1].apply(lambda x: 0 if x == 0 else 1)

    # Test split
    X_test = df_test.iloc[:, :-1]
    y_test = df_test.iloc[:, -1].apply(lambda x: 0 if x == 0 else 1)
...
if __name__ == "__main__":
    ...
    fit_evaluate(logreg, resnet_encoded_train, ptb_y_train,
                 resnet_encoded_test, ptb_y_test, num_classes=1,
                 epochs=50, batch_size=64, val_split=0.1)
    ...
    new_encoder.fit(ptb_X_train, ptb_y_train, epochs=50,
