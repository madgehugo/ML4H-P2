...
def build_resnet_encoder(input_shape, num_classes, filters=32,
...
if __name__ == "__main__":
    ...
    ae_encoder = build_resnet_encoder(input_shape, filters=32,
                                      kernel_size=5, strides=2,
                                      out_activation='sigmoid',
                                      num_classes=64)

    ae_decoder = build_decoder(num_classes=n_classes, latent_dim=latent_dim,
    ...
    autoencoder = Model(autoencoder_input, ae_decoded_output,
                        name='autoencoder')
    ...
    fit_evaluate(logreg, ae_encoded_train, ptb_y_train,
                 ae_encoded_test, ptb_y_test, num_classes=1,
                 epochs=50, batch_size=64, val_split=0.1)
    ...
    new_encoder.fit(ptb_X_train, ptb_y_train, epochs=50, batch_size=32,
